...
const BreadcrumbLink = React.forwardRef<HTMLAnchorElement, BreadcrumbLinkProps>(
  ({ asChild, className, children, ...props }, ref) => {
    const classes = cn(
      "text-[#64748B] dark:text-[#94A3B8] hover:text-[#0F172A] dark:hover:text-[#F8FAFC] transition-colors",
      className
    );
...
>(({ className, children, ...props }, ref) => (
  <li
    ...
    aria-hidden="true"
    className={cn(
      "text-[#94A3B8] dark:text-[#475569] [&>svg]:h-3.5 [&>svg]:w-3.5",
      className
    )}
    {...props}
  >
...
>(({ className, ...props }, ref) => (
  <span
    ...
    aria-hidden="true"
    className={cn(
      "flex h-9 w-9 items-center justify-center text-[#64748B] dark:text-[#94A3B8]",
      className
    )}
    {...props}
  >
